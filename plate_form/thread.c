...
void thread_handle_votary_fast_smc(struct thread_smc_args *args)
{
static int count=0;

//platform_optee(TEE_PROPSET_TEE_IMPLEMENTATION,"gpd.tee.description");

IMSG("Your handle thread_handle_votary_fast_smc called congrats !!!!!!!!!!: %d\n", count++);

 //platform_optee(TEE_PROPSET_TEE_IMPLEMENTATION,"gpd.tee.description");
/*os_info_structure_t *osinfo;
	DMSG("has been called");
	osinfo=PLAT_OPTEE;
	IMSG("osname->os_name =%s",osinfo->os_name);
	IMSG("osapiversion->os_apiversion =%s",osinfo->os_apiversion);
	IMSG("osversion->os_version =%s",osinfo->os_version);
	IMSG("osprovider->os_provider =%s",osinfo->os_provider);*/

thread_handle_fast_smc(args);
IMSG("************ END thread_handle_votary_fast_smc*******");
}
...
{
static int count=0;
IMSG("Your START std_smc handle is called congrats!!!!!!!: %d\n", count++);
...
IMSG("************ END thread_handle_STD_smc*******");
